...
def test_create_course(client, auth_token):
    response = client.post('/api/courses', headers={'Authorization': f'Bearer {auth_token}'}, json={
        'title': 'New Course',
...
def test_get_courses(client, auth_token):
    client.post('/api/courses', headers={'Authorization': f'Bearer {auth_token}'}, json={
        'title': 'Test Course',
        'description': 'Test Description'
    })
    response = client.get('/api/courses', headers={'Authorization': f'Bearer {auth_token}'})
    assert response.status_code == 200
    courses = response.get_json()
